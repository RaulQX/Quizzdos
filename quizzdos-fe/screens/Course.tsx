			...
			<ScrollView style={s`flex flex-col`}>
				{dummyDataLectures.lectures.map(
					({ name, summary, materials, quizzes }, key) => {
						const percentage =
							(quizzes.filter(({ completed }) => completed)
								.length /
								quizzes.length) *
							100
							...
							<View style={s`flex flex-col`} key={key}>
								...
								>
									<View style={s`flex-1 flex-col`}>
										<View
											style={s`flex flex-row align-center`}
										>
											<Text style={s`font-bold text-xl`}>
												{name}
											</Text>
											<View
												style={s`ml-3 bg-coolGray-700 px-2 py-1 rounded-md`}
											>
												<Text
									...
									<View style={s`flex justify-center`}>
										<View
											style={s`p-2 border-2 border-b-4 rounded-xl`}
										>
											<BookIcon
												style={s`w-6 text-white`}
											/>
										</View>
									</View>
								</View>
								<View
									style={s`flex flex-col items-center mb-2`}
								>
									{quizzes.map(
										({ type, name, completed }, key) => {
											const margin =
												Math.sin(
													(index * Math.PI) / 4
												) * 100
